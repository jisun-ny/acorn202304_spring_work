...
public class CafeServiceImpl implements CafeService {
	...
	// 하나의 서비스는 여러개의 dao에 의존할 수 있다.
	@Autowired
	private CafeDao cafeDao;
	@Autowired
	private CafeCommentDao cafeCommentDao;

	// 페이징 처리, 검색어 기능을 고려한 비지니스 로직 처리를 하는 메소드
	@Override
	public void getList(HttpServletRequest request, Model model) { 
		/*
		 * 원래 int pageNum이랑 Model model을 전달해주려했늗네 이러면 keyword, condition도 받아와야하고, 그러면 또 dto를 많이 건들여야하기때문에 후퇴..
		 */
		// 한 페이지에 몇개씩 표시할 것인지
		...
		//model에 정보를 담아준다.(model.addAttribute) --> request에 담아도 되지만 연습삼아 바꿔봄.
	...
	public void saveContent(CafeDto dto) {
		// title, content, writer 정보가 들어있는 CafeDto를 dao에 전달해서 DB에 저장되도록한다.
		cafeDao.insert(dto);
